...
export default function RagnarokMClassicPage() {
  ...
  // Find the active toon
  const activeToon = featuredToons.find((toon) => toon._id === activeTab)
        ...
        <div className="flex flex-col min-h-[calc(100vh-4rem)]">
          ...
          <main className="flex-1">
            <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8">
              ...
              <div className="rounded-lg border border-dashed border-border/60 bg-card/20 p-8 text-center">
                <h2 className="text-xl font-medium text-muted-foreground mb-2">Main Content Area</h2>
                <p className="text-muted-foreground">Game information and details will be displayed here.</p>
              </div>
            </div>
          </main>

          {/* Footer with Character Tabs */}
                      ...
                      <div className="flex rounded-md bg-muted/50 p-1">
                        {featuredToons.map((toon) => (
                          <button
                            ...
                            onClick={() => setActiveTab(toon._id as string)}
                          >
                            {toon.displayName}
                            {activeTab === toon._id && (
                              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary mx-3" />
                            )}
                          </button>
                        ))}
                      </div>
                      ...
                      {activeToon && (
                        <div className="absolute inset-0 flex justify-center items-center">
                          <button
                            ...
                            onClick={() => handleZoom(activeToon.imageUrl)}
                          >
